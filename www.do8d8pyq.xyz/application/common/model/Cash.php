...
class Cash extends Base {
    ...
    public function saveData($param)
    {
        ...
        $tx_points = intval($data['cash_money']);
        if($tx_points > $GLOBALS['user']['user_money']){
            return ['code'=>1007,'msg'=>'余额不足'];
        }
        ...
        //更新用户表
        $update=[];
        $update['user_money'] = $GLOBALS['user']['user_money'] - $tx_points;
        $update['user_money_froze'] = $GLOBALS['user']['user_money_froze'] + $tx_points;

        $where=[];
        $where['user_id'] = $GLOBALS['user']['user_id'];
        $res = model('user')->where($where)->update($update);
        if(false === $res){
            return ['code'=>1005,'msg'=>'更新用户余额失败：'.$this->getError() ];
        }
    ...
    public function delData($where)
    {
        ...
        foreach($list as $k=>$v){
            ...
            if($v['cash_status'] ==0){
                ...
                $update=[];
                $update['user_money'] = $user['user_money'] + $v['cash_points'];
                $update['user_money_froze'] = $user['user_money_froze'] - $v['cash_points'];
    ...
    public function auditData($where)
    {
        ...
        foreach($list as $k=>$v){
            ...
            $res = model('User')->where($where2)->setDec('user_money_froze', $v['cash_points']);
            if(false === $res){
                return ['code'=>1005,'msg'=>'更新用户余额失败：'.$this->getError() ];
            }

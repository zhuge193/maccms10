...
class Vod extends Base
{
    ...
    public function batch()
    {
        ...
        if (!empty($param)) {
            ...
            if(empty($param['ck_del']) && empty($param['ck_level']) && empty($param['ck_vip']) && empty($param['ck_status']) && empty($param['ck_lock']) && empty($param['ck_hits'])
            ...
            foreach($res['list'] as  $k=>$v){
				...
				if(!empty($param['ck_vip']) && !empty($param['val_vip'])){
                    $update['vod_vip'] = $param['val_vip'];
                    $des .= '&nbsp;'.lang('vip').'：'.$param['val_vip'].'；';
                }
    ...
    public function field()
    {
        ...
        if(!empty($ids) && in_array($col,['vod_status','vod_lock','vod_level','vod_vip','vod_hits','type_id','vod_copyright'])){
            $where=[];
            $where['vod_id'] = ['in',$ids];
            $update = [];
            if(empty($start)) {
